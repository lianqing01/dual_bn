...
class BatchNorm_mvv12d(_BatchNorm):
    """
    ...
    :class:`apex.parallel.BatchNorm_mvv12d` is designed to work with
    ...
    Example::
        >>> sbn = apex.parallel.BatchNorm_mvv12d(100).cuda()
    ...
    def __init__(self, num_features, eps=1e-5, momentum=0.2, affine=True, track_running_stats=True, channel_last=False):
        if channel_last == True:
            raise AttributeError("channel_last is not supported by primitive BatchNorm_mvv12d implementation. Try install apex with `--cuda_ext` if channel_last is desired.")

        if not BatchNorm_mvv12d.warned:
            if hasattr(self, "syncbn_import_error"):
                print("Warning:  using Python fallback for BatchNorm_mvv12d, possibly because apex was installed without --cuda_ext.  The exception raised when attempting to import the cuda backend was: ", self.syncbn_import_error)
            else:
                print("Warning:  using Python fallback for BatchNorm_mvv12d")
            BatchNorm_mvv12d.warned = True

        super(BatchNorm_mvv12d, self).__init__(num_features, eps=eps, momentum=momentum, affine=affine, track_running_stats=track_running_stats)
