...
class BatchNorm_oracle2d(_BatchNorm):
    """
    ...
    :class:`apex.parallel.BatchNorm_oracle2d` is designed to work with
    ...
    Example::
        >>> sbn = apex.parallel.BatchNorm_oracle2d(100).cuda()
    ...
    def __init__(self, num_features, eps=1e-4, momentum=0.1, affine=True, track_running_stats=True, channel_last=False):
        if channel_last == True:
            raise AttributeError("channel_last is not supported by primitive BatchNorm_oracle2d implementation. Try install apex with `--cuda_ext` if channel_last is desired.")

        if not BatchNorm_oracle2d.warned:
            if hasattr(self, "syncbn_import_error"):
                print("Warning:  using Python fallback for BatchNorm_oracle2d, possibly because apex was installed without --cuda_ext.  The exception raised when attempting to import the cuda backend was: ", self.syncbn_import_error)
            else:
                print("Warning:  using Python fallback for BatchNorm_oracle2d")
            BatchNorm_oracle2d.warned = True

        super(BatchNorm_oracle2d, self).__init__(num_features, eps=eps, momentum=momentum, affine=affine, track_running_stats=track_running_stats)
    ...
    def forward(self, input):
        ...
        if not self.training and self.track_running_stats:
            ...
            self.mu_dis = ((mean - self.running_mean)**2).mean()
            self.gamma_dis = ((var - self.running_var)**2).mean()
...
class SyncBatchnormFunction(Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_input = grad_mu_ = grad_gamma_ = None
        ...
        if ctx.needs_input_grad[0]:
            ...
            c_last_grad_input = (c_last_grad - mean_dy -\
                                 ...
                                 mean_dy_xmu  ) / torch.sqrt(var + eps)
